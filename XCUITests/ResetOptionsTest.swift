...
class ResetOptionsTest: BaseTestCase {
    ...
    func testResetLaunchOptions() {
        ...
        let topSites = app.tables["Top sites"].cells["TopSitesCell"].childrenMatchingType(.Other).matchingIdentifier("TopSite")
        let numberOfTopSites = topSites.count
        ...
        XCTAssertTrue(topSites.count == numberOfTopSites + 1, "A new site should have been added to the topSites")

        // Check to make sure topSites reset
        restart(app, reset: true)
        XCTAssertTrue(topSites.count == numberOfTopSites, "Only the default topSites should exist")
    }
